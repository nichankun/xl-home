...
interface PaketSectionProps {
  ...
  setKategori: (k: PaketKategori) => void;
  onPilihPaket: (nama: string) => void;
}

export function PaketSection({ paketData, kategori, setKategori, onPilihPaket }: PaketSectionProps) {
  return (
    <section className="py-20 bg-white" id="paket">
      <div className="max-w-295 mx-auto px-6">
        <div className="text-center">
          ...
          <h2 className="font-extrabold text-3xl sm:text-4xl text-[#0A0F1E] tracking-tight">Paket Sesuai <em className="not-italic text-[#0057FF]">Kebutuhan Anda</em></h2>
          <p className="text-base text-[#64748B] mt-3 max-w-130 mx-auto">Pilih paket yang paling sesuai. Semua paket sudah termasuk kuota HP untuk keluarga.</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-10">
          {paketData[kategori].map((p) => (
            <div className={`rounded-3xl p-8 border-2 relative overflow-hidden transition-all duration-300 flex flex-col ${
              p.popular ? "border-[#0057FF] bg-linear-to-b from-[#001A5C] to-[#0057FF] text-white shadow-[0_20px_60px_rgba(0,87,255,0.3)] hover:-translate-y-1.5" : "border-[#E2E8F4] bg-white text-[#0A0F1E] hover:-translate-y-1.5 hover:border-[#0057FF]"
            }`} key={p.nama}>
              {p.popular && <div className="absolute top-5 right-5 bg-linear-to-r from-[#FBBF24] to-[#F59E0B] text-[#78350F] text-[11px] font-bold px-3 py-1 rounded-full">⭐ Pilihan Terpopuler</div>}
                ...
                {p.benefits.map((b) => (
                  <li className="flex items-start gap-2.5 text-sm" key={b}>
                    <div className={`shrink-0 w-4.5 h-4.5 rounded-full flex items-center justify-center mt-0.5 ${p.popular ? "bg-white/20" : "bg-[#EEF3FF]"}`}>
                      <svg width="10" height="10" viewBox="0 0 10 10" fill="none"><path d="M2 5l2 2 4-4" stroke={p.popular ? "white" : "#0057FF"} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>
                    </div>
              ...
              <button
                className={`w-full mt-5 p-3.5 rounded-xl text-md font-semibold cursor-pointer border-none transition-all flex items-center justify-center gap-2 ${
                  p.popular ? "bg-white text-[#0057FF] hover:bg-white/90" : "bg-[#F8FAFF] text-[#0057FF] border border-[#E2E8F4] hover:bg-[#0057FF] hover:text-white"
                }`}
