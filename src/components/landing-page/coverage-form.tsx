...
export function CoverageFormSection({
  ...
  // FIX: Menangani event submit dari form (mendukung tombol Enter)
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (alamat.trim() && nomorWA.trim()) {
      onSubmit()
    }
  }
    ...
    >
      <div className="max-w-295 mx-auto px-6">
        <div className="max-w-150 mx-auto bg-white rounded-[28px] p-8 sm:p-11 shadow-[0_24px_80px_rgba(0,87,255,0.1)] border border-[#0057FF]/10">
          ...
          {!submitted ? (
            /* FIX: Mengubah Fragment (<>) menjadi <form> untuk semantik web & navigasi Enter */
            <form onSubmit={handleSubmit}>
              ...
              {selectedPaket && (
                <div className="flex items-center gap-2 bg-[#EEF3FF] border border-[#0057FF]/20 rounded-xl px-4 py-3 mb-6 text-sm font-semibold text-[#0057FF]">
                  {/* FIX: A11y pada icon */}
                  <Check aria-hidden="true" className="h-4 w-4 text-[#0057FF] shrink-0" />
                  <span>
                  ...
                  <Button
                    ...
                    // FIX: aria-label untuk pengguna screen reader
                    aria-label="Batalkan pilihan paket"
                    className="ml-auto h-6 w-6 rounded-full text-[#94A3B8] hover:bg-destructive/10 hover:text-destructive p-0 transition-colors"
                  >
                    {/* FIX: A11y pada icon */}
              ...
              {/* Tombol Submit */}
              <Button
                // FIX: Menambahkan type="submit" karena sekarang berada di dalam <form>
                type="submit"
                size="lg"
            ...
            <div className="text-center py-5">
              {/* FIX: A11y pada emoji dekoratif */}
              <div 
                role="img" 
                aria-label="Berhasil"
                className="w-16 h-16 rounded-full bg-[#D1FAE5] flex items-center justify-center mx-auto mb-4 text-3xl"
              >
                ✅
              </div>
